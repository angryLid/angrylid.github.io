...
- `src/pages/` - Astro page components and routing
  ...
  - `pres/[slug]/present/[pageNo].astro` - Dynamic slide route (one static HTML per slide)
  - `pres/[slug]/index.astro` - Redirects /pres/[slug]/ → /pres/[slug]/present/1/
  - `pres/[slug]/speaker.astro` - Speaker-view popup (pre-renders all slides; BroadcastChannel-synced to the main window)
...
- **Presentation Mode**: `/pres/[slug]/present/[pageNo]/` slides with arrow-key nav, CSS-driven animation steps, View Transitions, a Shift+S speaker-view popup with speaker notes, and BroadcastChannel dual-window sync (see Presentation Mode below)
...
- **Routes**: `/pres/[slug]/present/[pageNo]/` (one static HTML per slide), `/pres/[slug]/speaker/` (speaker-view popup), `/pres/[slug]/document/` (whole deck as a scrollable document, embedded in blog posts), `/pres/[slug]/` (redirect to slide 1), `/pres/` (listing)
...
- **Speaker view**: `/pres/[slug]/speaker/` pre-renders all slides in hidden `<section data-page>` containers; a `ResizeObserver` scales each preview via `transform: scale(var(--s))` against a 1280×720 design size. Speaker notes via the `<Notes>` MDX component (inert `<template>` on the audience side; `template.content.cloneNode(true)` into the notes panel on the speaker side). The page has no `<ClientRouter />` and no `astro:page-load` — the deferred module script initializes directly
...
- **Never run a build** (`pnpm build` / `astro build`). The user boots the dev server and prepares a Chrome instance for you. Just drive Chrome via the `chrome-devtools` MCP tools to visit `localhost:4321` and inspect/validate the running site. If any error occurs, report it to the user immediately.
- **Do not** automatically run `pnpm lint`, `pnpm typecheck`, or `pnpm format` after making changes. Only run these commands when explicitly asked to do so by the user.
