...
- Added presentation mode `/pres/[slug]/[pageNo]/`: MDX-per-slide (`pres` collection), arrow-key nav with `data-anim` build steps, View Transitions, Shift+Enter fullscreen
- Added presenter view (Shift+S): popup with scaled slide preview, speaker-notes teleprompter via `<Notes>` MDX component, next-slide thumbnail, bidirectional BroadcastChannel sync with the main window
